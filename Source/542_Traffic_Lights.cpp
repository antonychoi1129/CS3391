...
int main(){
	while(true){
		//init
		int signals[100]; int p = 0;
		int green[100]={0};
		
		//input
		while(~scanf("%i",&signals[p]))
			if(signals[p++]==0) break;	//terminated
		p-=1;
		
		if(signals[0]==0) break;	//Must be All 0
		...
		for(i=startSec; i <= f5hours; i++){
			...
			for(j=0;j<p;j++){
				while( i>= signals[j]-5+green[j] )
					green[j]+= signals[j]*2;
				if(!(green[j]<=i && i<green[j]+signals[j]-5)){
		...
		if(syncTime>f5hours) puts("Signals fail to synchronise in 5 hours");
